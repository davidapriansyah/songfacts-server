...
export class SongsService {
  ...
  async getRandomSong(excludeIds: number[] = []) {
  ...
  async saveFromYoutube(videoId: string) {
    ...
    if (videos.length === 0) {
      const response = await axios.get(
        `https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=${this.youtube['apiKey']}`,
      );
  ...
  async saveFromCache(videoId: string, title: string, artist: string, albumCover?: string) {
    const existingSong = await this.prisma.song.findFirst({
      where: { youtubeId: videoId },
    });
  ...
  private parseVideoTitle(videoTitle: string): { artist: string; title: string } {
    const separators = [' - ', ' – ', ' | ', ' by ', ': '];
  ...
  async getFunFacts(songId: number) {
    const song = await this.prisma.song.findUnique({
      where: { id: songId },
    });
    ...
    if (cachedFact) {
      const facts = cachedFact.funFacts as any;
      const hasRealData =
        facts &&
        typeof facts === 'object' &&
  ...
  async getRecommendations(songId: number, limit = 10) {
    const song = await this.prisma.song.findUnique({
      where: { id: songId },
    });
  ...
  async getMoodPlaylist(mood: string, limit = 20) {
  ...
  async updateYoutubeId(songId: number, youtubeId: string) {
    const song = await this.prisma.song.findUnique({
      where: { id: songId },
    });
  ...
  async updateLyrics(songId: number, lyrics: string) {
    const song = await this.prisma.song.findUnique({
      where: { id: songId },
    });
  ...
  async getGenreFromYoutube(genre: string, limit = 20) {
    const cacheKey = `genre:songs:${genre.toLowerCase()}`;
    const cached = await this.redisService.getJSON<{ cachedAt: number; songs: any[] }>(cacheKey);
    ...
    if (isFresh) {
      return (cached as any).songs.slice(0, limit);
    }
  ...
  private async searchGenreSongs(genre: string, limit = 20): Promise<any[]> {
    ...
    for (const item of items) {
      ...
      if (GENRE_BLACKLIST.some((w) => title.includes(w))) continue;
   ...
   */
  private async searchGenreSongsWithYtDlp(genre: string, limit = 20): Promise<any[]> {
    const searchQuery = GENRE_SEARCH_MAP[genre] || `${genre} music official audio`;
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (GENRE_BLACKLIST.some((w) => rawTitle.toLowerCase().includes(w))) continue;
        if (entry.live_status === 'is_live' || entry.is_live) continue;

        const parsed = this.parseVideoTitle(rawTitle);
        const existingSong = await this.prisma.song.findFirst({
          where: { youtubeId: videoId },
        });
        const thumbnail = entry.thumbnails?.[0]?.url || (existingSong?.albumCover as string) ||
  ...
  private async toGenreSong(existingSong: any, videoId: string, title: string, artist: string, thumbnail?: string): Promise<any> {
    return {
      ...(existingSong || {}),
      videoId,
  ...
  async getAiRecommendations(userId?: number) {
    ...
    const dbResults: any[] = dbSongs.map((s) => ({ ...s, inDb: true }));
    ...
    try {
      for (const q of searchQueries) {
        ...
        for (const item of items) {
          ...
          if (blacklisted.some((w) => title.includes(w))) continue;
    ...
    if (mixed.length < 10) {
      const remainingDbSongs = await this.prisma.song.findMany({
        select: { id: true, title: true, artist: true, genres: true, youtubeId: true, albumCover: true },
        where: { id: { notIn: [...seenIds].filter((id): id is number => typeof id === 'number') } },
        take: 15,
        orderBy: { title: 'asc' },
      });
  ...
  async getYoutubeRecommendations(songId: number) {
    ...
    if (cached) {
      const recs = cached.recommendations as any[];
      if (Array.isArray(recs) && recs.length > 0) {
        // Refresh inDb status for each cached recommendation
        const refreshed = await Promise.all(
          recs.map(async (rec) => {
            if (rec.inDb) return rec;
            const existingSong = await this.prisma.song.findFirst({
    ...
    // Fetch from YouTube
    const youtubeResults = await this.youtube.searchRelated(
      song.artist,
      song.genres,
      12,
    );
